...
import { AdInterest, SavedAudience } from "./type";
...
export async function getListAdInterest(label: string, limit: number): Promise<AdInterest[]> {
  try {
    const session = await getSession();
    console.log("Session in getSavedAudiences:", session);
    ...
    const url = `https://graph.facebook.com/v24.0/search?type=adinterest&q=${label}&limit=${limit}&access_token=${accessToken}`;
